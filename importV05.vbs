...
'* 2015-12-30  Changheng GU     add Code Merge 4.6 compatibility
'*
'|**********************************************************************;
...
ElseIf (cellString = "Code Merge 4.6") Then
    Worksheets("Sheet1").Range("P" & iParam).Value = "MRG"
    Worksheets("Sheet1").Range("T" & iParam).Value = "software"
...
If (jiraId Like "AE-*") Then
    Worksheets("Sheet1").Range("S" & iParam).Value = "5.0 Defects Carry over"
End If
...
'#########################
'cleanup function handles unexpected tickets after the whole modify process
'#########################
