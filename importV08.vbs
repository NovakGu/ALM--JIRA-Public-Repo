...
'* 2015-02-01  Changheng GU             Changed Defect Type for Code Merge Sub-tasks
'*
'|**********************************************************************;
...
defect_type = "Bug"
Dim sub_task As String
sub_task = "Development Task"
...
'Override all defect type as Bug
    Dim row As Integer
        For row = 2 To k
            If Not IsEmpty(Cells(row, 16).Value) And Cells(row, 15).Value = "Code Merge 4.6" Then
                Cells(row, 7).Value = sub_task
            Else
